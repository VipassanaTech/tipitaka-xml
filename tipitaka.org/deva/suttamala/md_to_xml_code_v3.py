...
def process_numbered_heading(line):
    ...
    Checks if the given line matches the pattern:
      ** @ [Devanagari Number].[rest of text]**
    If it matches:
      - Extracts the Devanagari number and the text after the dot.
      - Converts Devanagari number to English.
    ...
    #pattern = r'^\*\*\s@\s([१२३४५६७८९०]+)\.\s*(.*?)\s@\s\*\*$'
    pattern = r'^\*\*\s@([१२३४५६७८९०]+)\.\s(.*)\*\*'

    match = re.match(pattern, line.strip())
    if match:
        # Extract captured groups
        devanagari_num = match.group(1)
        ...
        # If the pattern does not match, return the line as is
